...
@api_router.post("/admin/emergency-reset-user-password")
async def emergency_reset_user_password(email: str, new_password: str):
    """Emergency endpoint to reset user password and recreate missing users if needed"""
    
    # Check if user exists
    user = await db.users.find_one({"email": email})
    
    if not user:
        print(f"🚨 EMERGENCY: User {email} not found - creating new user")
        
        # Create the missing user with minimal data
        ...
        new_user = User(
            ...
            email_verified=True,  # Auto-verify for emergency users
            is_admin=False,
            balance=0.0
        )
        ...
        print(f"✅ EMERGENCY: Created new user {email} with password reset")
        return {
            "message": f"User {email} was missing and has been recreated with new password",
            "action": "user_recreated",
            "email": email,
            "password_set": True
        }
    else:
        ...
        print(f"✅ EMERGENCY: Reset password for {email}")
        return {
...
@api_router.post("/admin/emergency-restore-users")
async def emergency_restore_users():
    """Emergency endpoint to restore the 3 affected users with their original passwords"""
    
    users_to_restore = [
        {"email": "Leandro.miguel.360@hotmail.com", "password": "91608340", "name": "Leandro Hotmail"},
        {"email": "Leandro.miguel.360@gmail.com", "password": "91608340", "name": "Leandro Gmail"}, 
        {"email": "j.cs26@hotmail.com", "password": "91608340", "name": "J CS26"}  # Assuming same password
    ]
    ...
    for user_info in users_to_restore:
        ...
        if not user:
            ...
            await db.users.insert_one(new_user.dict())
            results.append(f"✅ Created missing user: {user_info['email']}")
        else:
            ...
            results.append(f"✅ Reset password for existing user: {user_info['email']}")
    
    return {
        "message": "Emergency user restoration completed",
        "results": results,
        "users_processed": len(users_to_restore)
    }
...
async def get_all_bets():
    """Get all bets with legacy compatibility"""
